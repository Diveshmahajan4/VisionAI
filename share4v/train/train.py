...
def get_peft_state_non_lora_maybe_zero_3(named_params, require_grad_only=True):
    to_return = {
        k: t
        for k, t in named_params
        if ("lora_" not in k) and ("vision_tower" not in k)
    }
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if getattr(trainer.args, "tune_vision_tower", False) or getattr(
        trainer.args, "tune_entire_model", False
    ):
        if trainer.deepspeed:
            torch.cuda.synchronize()
        trainer.model.get_vision_tower().image_processor.save_pretrained(
...
def unlock_vit(training_args, model_args, vision_tower):
    lr_of_vit = (
        training_args.vision_tower_lr
        if training_args.vision_tower_lr is not None
        and training_args.vision_tower_lr != 0
    ...
    for n, p in vision_tower.named_parameters():
        if model_args.tune_vit_from_layer != -1:
            if "vision_tower.vision_model.encoder.layers." in n:
                layer_id = int(
                    n.split("vision_tower.vision_model.encoder.layers.")[-1].split(".")[
                        0
                    ]
                )
...
def train():
    ...
    if model_args.vision_tower is not None:  # Start the vision encoder initialization
        ...
        model.config.tune_entire_model = (
            training_args.tune_entire_model
        ...
        model.config.tune_vision_tower = (
